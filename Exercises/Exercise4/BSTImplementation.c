...
/*----------------------------------------------------------------------------*/
void _inOrderTraversal(struct Node*, TYPE, int*);
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	*found = 0;
	...
	if (tree->cnt == 0)
		...
		_inOrderTraversal(tree->root, val, found);

	return *found;
								
}

// Recursive helper function that traverses the tree in order and
// sets found = 1 if it finds the value.
void _inOrderTraversal(struct Node* traversal, TYPE val, int* found)
{
	if (traversal != 0)
	{
		_inOrderTraversal(traversal->left, val, found);

		if (compare(traversal->val, val) == 0)
			*found = 1;

		_inOrderTraversal(traversal->right, val, found);
	}
}
