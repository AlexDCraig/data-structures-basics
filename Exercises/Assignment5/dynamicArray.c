...
int _smallerIndexHeap(DynArr *heap, int i, int j, comparator compare)
{
	...
	{
		printf("\nIndex provided is larger than heap size.\n");
		exit(1);
	}

	
	if ((*compare)(getDynArr(heap, i), getDynArr(heap, j)) == -1) // i is the smaller index
		return i;
...
TYPE getMinHeap(DynArr *heap)
{
	if (heap->size == 0)
	{
		printf("\nNothing in the heap. Can't get min.\n");
		exit(1);
	}

	else
		return getDynArr(heap, 0); // returns task struct stored at root
}
...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
	...
	addDynArr(heap, val); // add node to DynArr->size. inc DynArr->size 

	if (heap->size > 1)
	{
		int recentlyAddedElement = heap->size - 1;
		int parentOfElement = (recentlyAddedElement - 1) / 2;
		
		while ((*compare)(getDynArr(heap, recentlyAddedElement), getDynArr(heap, parentOfElement)) == -1) // node is less than parent
		{	
			swapDynArr(heap, recentlyAddedElement, parentOfElement); // swap the data members of the recent element and its father
			recentlyAddedElement = parentOfElement; // we swapped them so now the recently added element 
			parentOfElement = (recentlyAddedElement - 1) / 2;
		}
	}		
}
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
	...
	if (rightChild < max) // we have two kids
	{
		int smallestChildIndex = _smallerIndexHeap(heap, leftChild, rightChild, compare);
		
		if ((*compare)(getDynArr(heap, pos), getDynArr(heap, smallestChildIndex)) == 1) // position is larger than smallestChildIndex
		{
			swapDynArr(heap, pos, smallestChildIndex);
			_adjustHeap(heap, max, smallestChildIndex, compare);
		}
	
	}

	else if (leftChild < max) // we have one kid
	{
		if ((*compare)(getDynArr(heap, pos), getDynArr(heap, leftChild)) == 1) // position is larger than leftchild
		{
...
void removeMinHeap(DynArr *heap, comparator compare)
{
	...
	putDynArr(heap, 0, getDynArr(heap, last)); // Transfer last filled element to root
	removeAtDynArr(heap, last); // Remove the root, which has been moved to the last element
	_adjustHeap(heap, last, 0, compare); // Percolate downwards
}
